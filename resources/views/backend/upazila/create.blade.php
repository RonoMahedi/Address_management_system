            ...
            <div class="card-header">
              <h3>
                ...
                @else
                  Add Upazila
                @endif
                <a href="{{route('upazilas.index')}}" class="btn btn-success float-right btn-sm"> <i class="fa fa-plus-circle">Upazila List</i></a>
              </h3>
            </div><!-- /.card-header -->
            <div class="card-body">
                <form class="" action="{{(@$editData)?route('upazilas.update',$editData->id):route('upazilas.store')}}" method="post" id="myForm">
                  @if(@$editData)
                      @method("put")
                  @else
                    @method("post")
                    ...
                    <div class="form-group col-md-6">
                      ...
                      <select class="form-control" name="division_id" id="division_id">
                        ...
                        @foreach($districts as $district)
                          <option value="{{$district->id}}" {{(@$editData->district_id==$district->id)?"selected":""}}>{{$district->name}}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="form-group col-md-6">
